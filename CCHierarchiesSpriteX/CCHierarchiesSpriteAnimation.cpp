...
void CCHierarchiesSpriteAnimation::parseItems (rapidxml::xml_node<>* itemsNode) {
	// <Item name=S left=N bottom=N dyeingPartItemIndex=I>
	...
	while (itemNode) {
		...
		attribute = itemNode->first_attribute("dyeingPartItemIndex");
		if (NULL == attribute) {
			CCLOG("parse <Item> Node <dyeingPartItemIndex> Attr error");
			return;
		}
		int Item_dyeingPartItemIndex = atoi(attribute->value());
		
		_items.push_back(Item(Item_name, Item_left, Item_bottom, Item_dyeingPartItemIndex));
...
void CCHierarchiesSpriteAnimation::parseAnimations (rapidxml::xml_node<>* animationsNode) {
	// <Animation name=S startFrameIndex=N endFrameIndex=N loop=B>
	...
	while (animationNode) {
		...
		attribute = animationNode->first_attribute("loop");
		if (NULL == attribute) {
			CCLOG("parse <Animation> Node <loop> Attr error");
			return;
		}
		...
		std::pair<std::string, Animation> item(Animation_name, Animation(Animation_name, Animation_startFrameIndex, Animation_endFrameIndex, Animation_loop));
        _anims.insert(item);
